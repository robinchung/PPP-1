// MeterPressure.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterPressure, CStatic)

MeterPressure::MeterPressure()
{
...
/*	m_colorTable[0] = RGB(255, 255,   0);
	...
	m_colorTable[0] = RGB(255, 255,   0);
	m_colorTable[1] = RGB(  255, 255,   0);
	m_colorTable[2] = RGB(0,   255,   0);
	m_colorTable[3] = RGB(0,255, 0);
	m_colorTable[4] = RGB(255, 0, 0);
...
BEGIN_MESSAGE_MAP(MeterPressure, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()

void MeterPressure::OnPaint() 
{
...
void MeterPressure::DrawMeterBackground(CDC *pDC, CRect &rect)
{
	...
	/*m_font.CreateFont(nHeight, 0, 0, 0, 400, 
							...
							CLIP_DEFAULT_PRECIS,
							DEFAULT_QUALITY, 
							DEFAULT_PITCH|FF_SWISS, "Arial");*//////////////////////////////yingmou
	...
	{	
		...
		//strtemp.Format("%.*lf", m_nScaleDecimals, (m_dMinValue+(m_dMaxValue-m_dMinValue)*i/m_nTicks));////////////////////////////////////////////////yingmou
...
void MeterPressure::ReconstructControl()
{
...
void MeterPressure::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterPressure::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterPressure::DrawNode(CDC *pDC)
{
	CPen penDraw, *pPenOld;
...
void MeterPressure::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
...
//	strtemp.Format("%.*lf", m_nValueDecimals, m_dCurrentValue); ///////////////////////////////////yingmou
	m_font.DeleteObject() ;
	/*m_font.CreateFont (nHeight, 0, 0, 0, 400,
						FALSE, FALSE, 0, ANSI_CHARSET,
						OUT_DEFAULT_PRECIS, 
...
void MeterPressure::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterPressure::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterPressure::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterPressure::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterPressure::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterPressure::SetValueDecimals(int nDecimals)
{
...
void MeterPressure::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
...
// MeterPressure message handlers
