// MeterOil.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterOil, CStatic)
...
BEGIN_MESSAGE_MAP(MeterOil, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()

void MeterOil::OnPaint() 
{
...
void MeterOil::DrawMeterBackground(CDC *pDC, CRect &rect)
{
...
void MeterOil::ReconstructControl()
{
...
void MeterOil::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterOil::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterOil::DrawNode(CDC *pDC)
{
	CPen penDraw, *pPenOld;
...
void MeterOil::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
...
void MeterOil::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterOil::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterOil::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterOil::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterOil::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterOil::SetValueDecimals(int nDecimals)
{
...
void MeterOil::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
