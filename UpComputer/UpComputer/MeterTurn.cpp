// MeterTurn.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterTurn, CStatic)
...
BEGIN_MESSAGE_MAP(MeterTurn, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()

// MeterTurn message handlers

void MeterTurn::OnPaint() 
{
...
void MeterTurn::DrawMeterBackground(CDC *pDC, CRect &rect)
{
...
void MeterTurn::ReconstructControl()
{
...
void MeterTurn::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterTurn::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterTurn::DrawNode(CDC *pDC)
{
	CPen penDraw, *pPenOld;
...
void MeterTurn::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
...
void MeterTurn::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterTurn::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterTurn::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterTurn::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterTurn::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterTurn::SetValueDecimals(int nDecimals)
{
...
void MeterTurn::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
