// MeterTemperature.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterTemperature, CStatic)
...
BEGIN_MESSAGE_MAP(MeterTemperature, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()


// MeterTemperature message handlers

void MeterTemperature::OnPaint() 
{
...
void MeterTemperature::DrawMeterBackground(CDC *pDC, CRect &rect)
{
...
void MeterTemperature::ReconstructControl()
{
...
void MeterTemperature::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterTemperature::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterTemperature::DrawNode(CDC *pDC)
{
	CPen penDraw, *pPenOld;
...
void MeterTemperature::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
	...
	strtemp.Format(_T("%.*lf"), m_nValueDecimals, m_dCurrentValue); ///////////////////////////////////yingmou
	m_font.DeleteObject() ;
	m_font.CreateFont (nHeight, 0, 0, 0, 400,
						FALSE, FALSE, 0, ANSI_CHARSET,
						OUT_DEFAULT_PRECIS, 
...
void MeterTemperature::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterTemperature::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterTemperature::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterTemperature::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterTemperature::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterTemperature::SetValueDecimals(int nDecimals)
{
...
void MeterTemperature::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
