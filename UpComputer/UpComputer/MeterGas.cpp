// MeterGas.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterGas, CStatic)
...
BEGIN_MESSAGE_MAP(MeterGas, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()
...
// MeterGas message handlers

void MeterGas::OnPaint() 
{
...
void MeterGas::DrawMeterBackground(CDC *pDC, CRect &rect)
{
...
void MeterGas::ReconstructControl()
{
...
void MeterGas::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterGas::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterGas::DrawNode(CDC *pDC)
{
	CPen penDraw, *pPenOld;
...
void MeterGas::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
...
void MeterGas::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterGas::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterGas::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterGas::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterGas::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterGas::SetValueDecimals(int nDecimals)
{
...
void MeterGas::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
